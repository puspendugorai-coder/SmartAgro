function requestLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      if (btn) { btn.innerHTML='<i class="fas fa-check"></i> Location Found'; btn.style.background='linear-gradient(135deg,#166534,#22c55e)'; }
      loadDashboard(pos.coords.latitude, pos.coords.longitude);
    },
    () => {
      showToast('Using Delhi as default.','warning');
      if (btn) { btn.innerHTML='<i class="fas fa-location-crosshairs"></i> Get My Location'; btn.disabled=false; }
      loadDashboard(28.6139, 77.2090);
    },
...
async function loadDashboard(lat, lon) {
  ...
  if (card) card.innerHTML='<div class="hwc-loading"><div class="loading-spinner" style="width:32px;height:32px;margin:0 auto 8px"></div><span>Loading weather...</span></div>';
...
function renderHeroCard(w) {
  ...
  card.innerHTML = `
    <div class="hwc-loaded">
      <div class="hwc-city"><i class="fas fa-location-dot" style="color:var(--green)"></i> ${w.city}</div>
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div><div class="hwc-temp">${w.temp}°</div><div class="hwc-desc">${capitalize(w.description)}</div></div>
...
function renderWeatherSection(current, forecast) {
  ...
  if (main) main.innerHTML=`
    <div class="weather-primary-card">
      <div style="font-size:4.5rem">${getWeatherEmoji(current.icon)}</div>
      <div>
    ...
    <div class="weather-stat-card"><div class="wsc-icon"><i class="fas fa-droplets"></i></div><div class="wsc-label">Humidity</div><div class="wsc-val">${current.humidity}<span class="wsc-unit">%</span></div></div>
    <div class="weather-stat-card"><div class="wsc-icon"><i class="fas fa-wind"></i></div><div class="wsc-label">Wind</div><div class="wsc-val">${current.wind_speed}<span class="wsc-unit"> m/s</span></div></div>
    <div class="weather-stat-card"><div class="wsc-icon"><i class="fas fa-eye"></i></div><div class="wsc-label">Visibility</div><div class="wsc-val">${current.visibility.toFixed(1)}<span class="wsc-unit"> km</span></div></div>`;
  ...
  if (fg && forecast) {
    ...
    fg.innerHTML = forecast.map((day,i) => `
      <div class="forecast-card ${day.date===todayStr?'today':''}" style="animation-delay:${i*0.06}s">
        ...
        <div class="fc-temps"><span class="fc-max">${Math.round(day.temp_max)}°</span><span class="fc-min">${Math.round(day.temp_min)}°</span></div>
        <div style="font-size:0.68rem;color:var(--text-3);margin-top:4px"><i class="fas fa-droplets" style="color:#38bdf8"></i> ${day.humidity}%</div>
      </div>`).join('');
  }
}
...
function renderRainForecast(forecast) {
  ...
  const todayRain    = today.rain > 0 || today.description?.includes('rain');
  const tomorrowRain = tomorrow.rain > 0 || tomorrow.description?.includes('rain');
  ...
  grid.innerHTML = `
    <div class="rain-card ${todayRain?'rain-yes':'rain-no'}">
      ...
      <div class="rain-answer">${todayRain?'YES — Rain expected':'NO — No rain'}</div>
    ...
    <div class="rain-card ${tomorrowRain?'rain-yes':'rain-no'}">
      ...
      <div class="rain-answer">${tomorrowRain?'YES — Rain expected':'NO — No rain'}</div>
