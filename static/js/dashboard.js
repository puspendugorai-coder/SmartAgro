...
async function loadDashboard(lat, lon) {
  ...
  if (card) card.innerHTML = `<div class="hwc-loading"><div class="loading-spinner" style="width:28px;height:28px;margin:0 auto 8px;border-color:rgba(255,255,255,0.3);border-top-color:#fff"></div><span style="font-size:0.83rem">Loading weather...</span></div>`;
    ...
    fg.innerHTML = forecast.map((day, i) => `
      <div class="forecast-card ${day.date === todayStr ? 'today' : ''}" style="animation-delay:${i*0.06}s">
        ...
        <div style="font-size:0.65rem;color:var(--text-3);margin-top:3px">
          <i class="fas fa-droplets" style="color:#3b82f6"></i> ${day.humidity}%
        </div>
      </div>`).join('');
  }
}
