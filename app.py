...
{"disease":"name","confidence":85,"severity":"Mild/Moderate/Severe","affected_part":"Leaves/Stem/Fruit","cause":"pathogen description","eco_remedies":[{"remedy":"name","method":"how to apply","frequency":"how often","effectiveness":80}],"chemical_remedies":[{"name":"chemical","dose":"dose/L","interval":"days"}],"prevention":["tip1","tip2","tip3"],"recovery_timeline":"2-4 weeks"}"""
    for model in ["meta-llama/llama-4-scout-17b-16e-instruct","meta-llama/llama-4-maverick-17b-128e-instruct"]:
        try:
            ...
            clean = re.sub(r"```(?:json)?", "", raw).replace("
```", "").strip()
...
# ── Dynamic Advisories & Alerts Module ───────────────────
...
# ── Mandi Market Price Generation Engine ─────────────────
BASE_PRICES = {
    ...
    for city in CITIES:
        ...
        for crop, base in BASE_PRICES.items():
            ...
            crops.append({
                ...
                "unit":   "quintal",
                "change": change,
                "medium_demand": "Very High" if change > 2 else "High" if change > 0 else "Medium" if change > -2 else "Low"
            })
        markets[city] = crops
...
# ── Multi-lingual Voice Chatbot Endpoint ──────────────────
...
def chat():
    ...
    lang_names = {
        ...
        "te": "Telugu (తెలుగు)", "mr": "Marathi (मராठी)", "pa": "Punjabi (ਪੰਜਾਬੀ)",
        "gu": "Gujarati (ગુજરાਤੀ)", "kn": "Kannada (ਕੰਨੜ)", "ml": "Malayalam (മലയാളം)", "en": "English"
    }
...
You MUST reply completely and exclusively in the {lang_name} language using its native regional script characters (e.g. write in Devnagari if Hindi, Bengali characters if Bengali script). Never answer in English if the target language is different.
Keep answers SHORT, SIMPLE, and POINT-WISE (Max 3-4 bullet points) — farmers need easy, memorizable advice.
{weather_context}
...
if __name__ == "__main__":
    app.run(debug=True, port=5000)from flask import Flask, render_template, request, jsonify
import requests
import os
import json
import re
